package com.ltts.shopping.Entity;
...
public class ShoppingCart {
    ...
    public boolean add_item(Collection<? extends Items> item){

        int pos=check_item(((Items) item).getName());
        if(pos<0){
            this.items.addAll(item);
            return true;
        }else {
            System.out.println("Item => "+((Items) item).getName()+" already exist");
        }
        return false;
    }
	...
	public void add_item(com.ltts.Shoppingcart.Entity.Items item) {
